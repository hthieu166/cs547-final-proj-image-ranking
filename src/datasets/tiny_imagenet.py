...
class TinyImageNetDataset(Dataset):
    def __init__(self, mode, data_root, triplets_dir, datalst_pth = None, transform = None):
        self.mode = mode
        ...
        if self.mode == "train":
            ...
            with open(osp.join(triplets_dir, "val_cls.txt")) as fi:
                self.imgs_cls  = np.array([int(i.strip()) for i in fi.readlines()])
        else:
...
class TinyImageNetTripletDataset(TinyImageNetDataset):
    """TinyImageNet dataset with triplet-pairs selection"""  
...
if __name__ == "__main__":
    ...
    dataset = TinyImageNetTripletDataset(
        "train", "/home/hthieu/data/tiny-imagenet-200", "triplet_pairs",
        transform = transforms.ToTensor())
    data, lbl = next(iter(dataset))
    print(data.shape, lbl)
