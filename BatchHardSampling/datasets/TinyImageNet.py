...
class TinyImageNet(Dataset):
    '''
    a wrapper of TinyImageNet dataset
    '''
    def __init__(self, data_path, is_train = True, *args, **kwargs):
        ...
        self.imgs = self.getimages(data_path)#os.listdir(data_path)
        #self.imgs = [el for el in self.imgs if os.path.splitext(el)[1] == '.JPEG']
        ...
        for subdirs, dirs, files in os.walk(folderpath):
            for file in files:
                if (os.path.splitext(file)[1] == '.JPEG'):
                    ...
                    self.lb_ids.append(file_name.split('_')[0])
